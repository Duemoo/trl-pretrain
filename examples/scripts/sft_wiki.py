...
class ScriptArguments:
    ...
    eval_fpath: Optional[str] = field(default="/data/hoyeon/trl-pretrain/custom_knowledge/wikipedia_probe.json", metadata={"help": "Eval fpath"})
...
if script_args.mixed_train:
    ...
    fpath='/data/hoyeon/trl-pretrain/custom_knowledge/0708_4000.json'
    with open(fpath, 'r') as f:
        pre = json.load(f)
    texts=[p['text'] for p in pre]
    train_dataset = Dataset.from_dict({"text": texts})

else:
    fpath='/data/hoyeon/trl-pretrain/custom_knowledge/0607.json'
    with open(fpath, 'r') as f:
        pre = json.load(f)
    texts=[p['text'] for p in pre]
    train_dataset = Dataset.from_dict({"text": texts})
